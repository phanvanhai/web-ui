...
orgEdgexFoundry.deviceService = (function() {
    ...
    DeviceService.prototype = {
        ...
        renderListObjectForElement: null,
    ...
    DeviceService.prototype.showParameterOfElement = function() {
        var devicename = $("select[id='selectElementElement']").val();
        $.ajax({
            url: '/object/api/v1/object/name/' + devicename + '/commandlist',
            type: 'GET',
            success: function(data) {
                $("select[id='add_element_command']").empty();
                $.each(data, function(i, v) {
                    if (v.put != null) {
                        $("select[id='add_element_command']").append($("<option></option>")
                            .attr("value", v.name).text(v.name));
                    }
                });
            }
        });
    ...
    DeviceService.prototype.addElement = function() {
        orgEdgexFoundry.deviceService.renderListObjectForElement();
        $("#add-device-btn").hide();
        $(".edgexfoundry-device-command").hide();
        $(".edgexfoundry-device-element").hide();
        $(".edgexfoundry-device-schedule").hide();
    ...
    DeviceService.prototype.renderListObjectForElement = function() {
        //initialize loading device-ComboGrid data.        
        $.ajax({
            url: '/object/api/v1/object',
            type: 'GET',
            success: function(data) {
                console.log("initialize loading device-ComboGrid data.");
                orgEdgexFoundry.deviceService.deviceDataCache = data;
                $("select[name='list_object']").empty();
                $.each(data, function(i, d) {
                    $("select[name='list_object']").append($("<option></option>")
                        .attr("value", d.name).text(d.name));
                });
            }
        });
    }
    ...
    DeviceService.prototype.editDevice = function(device) {
        ...
        $('#idDeviceLabels option').removeAttr('selected').filter('[value=\'' + device.labels[0] + '\']').attr('selected', true);
        $("select[name='deviceLabels']").prop('disabled', true);
    ...
    DeviceService.prototype.addDevice = function() {
        ...
        $('#idSelectDSInAddObject option').removeAttr('selected').filter("[value=DeviceType]").attr('selected', true);
        $("select[name='deviceLabels']").prop('disabled', false);
    ...
    DeviceService.prototype.uploadDevice = function(type) {
        ...
        var arrLabels = $("select[name='deviceLabels']").val().split(",");
        var nameDSSelected = $("select[name='nameSelectDSInAddObject']").val();
        var nameProfileSelected = $("select[name='nameSelectProfileInAddObject']").val();
